        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Stage and Output\n" +
                    "Cues bind to displays by position on the canvas, not a DisplayId field. Output is any extra OS screen (Win+P Extend). Pick the wall/TV, not the Producer laptop. Esc closes Output. A D3D11 compositor decodes each H.264 file once (Media Foundation / DXVA) and draws the wall from that texture. While Output is live, Stage stays a labeled canvas (Display 1 in the center) so the laptop does not run a second 4K decode — that is why Stage looks black. An amber preview box still follows the cue so you can move and resize it. Close Output to preview video on Stage again. Output fills the HDMI/DP screen in real pixels. The Stage display stretch-fills that wall (1920×1080 Stage on a 1920×1080 TV is 1:1). If Log shows Output wall 1920×1080 · Stage display 3840×2160, Windows is not in 4K — Use size after setting the extra screen to 3840×2160 in Display settings. Snap on Stage uses a small magnet for drag/resize; dropping a clip onto a display still uses the larger magnet.\n\n" +
