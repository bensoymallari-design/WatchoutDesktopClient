        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Stage and Output\n" +
                    "Cues bind to displays by position on the canvas, not a DisplayId field. Output is any extra OS screen (Win+P Extend). Pick the wall/TV, not the Producer laptop. Esc closes Output. A D3D11 compositor decodes each H.264 file once (Media Foundation / DXVA) and draws the wall from that texture. While Output is live, Stage stays a labeled canvas (Display 1 in the center) so the laptop does not run a second 4K decode — that is why Stage looks black. An amber preview box still follows the cue so you can move and resize it. Drag the clip itself — Edit cues grabs the media, not the display title strip. Select the cue and press Fit cue (or Properties → Fit cue to display) so the whole picture sits on Display 1 without cropping. Type Width/Height in Properties for a custom size. Close Output to preview video on Stage again. Output contain-fits the Stage canvas onto the live wall/TV pixels — a 1920×1080 Stage fills a Colorlight X20 at 516×430, a 1080 TV, or 4K, without copying the controller onto Stage. Log: Output wall … · Stage display … · cue …. 6720×1344 in NVIDIA is leftover from another controller; for this X20 set X20 HDMI to 516×430 (Customize… if it is missing), 100% scale. Snap on Stage uses a small magnet for drag/resize; dropping a clip onto a display still uses the larger magnet.\n\n" +
