        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Import of a file video (HandBrake MP4, YouTube yt-dlp, ProRes) encodes HAP Q in the background like Resolume Alley — Play is a GPU DXT blit, not a 4K RAM copy. Encode no longer dies with “thread owns it”; ffmpeg progress is not dumped into the Log. Stage is a 1280 preview of the same GPU stack (Output stays 4K). Assets → Encode HAP does the same on a clip already in the list. Notch LC still transcodes to H.264 if HAP encode is not available. WAV files can declare up to 65535 channels; WatchMe downmixes wide files to stereo for playback. Export LTC WAV from the timeline playhead; ChaseLtc follows SMPTE time. Win+Shift+S / Print Screen used to close WatchMe when GPU memory was full — that error is caught and Play continues.\n\n" +
