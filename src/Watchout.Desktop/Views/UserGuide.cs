        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Stage and Output\n" +
                    "Cues bind to displays by position on the canvas, not a DisplayId field. Output is any extra OS screen (Win+P Extend). Pick the wall/TV, not the Producer laptop. Esc closes Output. A D3D11 compositor decodes each H.264 file once (Media Foundation / DXVA) and draws the wall from that texture. While Output is live, Stage stays a labeled canvas (Display 1 in the center) so the laptop does not run a second 4K decode — that is why Stage looks black. An amber preview box still follows the cue so you can move and resize it. Drag the clip itself — Edit cues grabs the media, not the display title strip. Select the cue and press Fit cue (or Properties → Fit cue to display) so the whole picture sits on Display 1 without cropping. Drag or resize the cue on Stage and Properties X/Y/Width/Height follow. Type Width/Height in Properties for a custom size. Close Output to preview video on Stage again. Output fills the HDMI/DP screen in the Windows mode pixels. An NVIDIA custom resolution (3160×2160) that you apply in Windows Display on the second monitor is copied onto Stage as 3160×2160 — not the controller EDID. Find screens, then Assign screens or Use size. A 3840 Stage cue used to look zoomed when the wall HWND was smaller (150% DPI or a 1080/1440 mode) — the swap now matches that window and contain-fits the display. Log: Output wall … · Stage display … · cue …. Snap on Stage uses a small magnet for drag/resize; dropping a clip onto a display still uses the larger magnet.\n\n" +
