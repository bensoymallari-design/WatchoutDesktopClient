        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Stage and Output\n" +
                    "Cues bind to displays by position on the canvas, not a DisplayId field. Output is any extra OS screen (Win+P Extend). Pick the wall/TV, not the Producer laptop. Esc closes Output. A D3D11 compositor decodes each H.264 file once (Media Foundation / DXVA) and draws the wall from that texture. While Output is live, Stage stays a labeled canvas (Display 1 in the center) so the laptop does not run a second 4K decode — that is why Stage looks black. An amber preview box still follows the cue so you can move and resize it. Drag the clip itself — Edit cues grabs the media, not the display title strip. Select the cue and press Fit cue (or Properties → Fit cue to display) so the whole picture sits on Display 1 without cropping. Type Width/Height in Properties for a custom size. Close Output to preview video on Stage again. Output fills the HDMI/DP screen in the Windows mode pixels. A 3840 Stage cue used to look zoomed when the wall HWND was smaller (150% DPI or a 1080/1440 mode) — the swap now matches that window and contain-fits the display. Log: Output wall … · Stage display … · cue …. If wall is 1920×1080 and display is 3840×2160, set the extra screen to 3840×2160 in Display settings, then Use size. Colorlight X20 NVIDIA customs (6720×1344, 5280×816, 1248×603, 516×430) are that live Windows mode — the panel EDID often still says 1920×1080 or 4096×2160. Devices lists 6720×1344 (EDID 1920×1080); Use size copies 6720×1344 onto Stage and Output presents at that size. A 1920×1080 cue on a 1920×1080 Stage cannot fill a 6720-wide wall — Use size, then Fit cue. Snap on Stage uses a small magnet for drag/resize; dropping a clip onto a display still uses the larger magnet.\n\n" +
