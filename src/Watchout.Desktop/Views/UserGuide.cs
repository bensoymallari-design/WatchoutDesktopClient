        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Stage and Output\n" +
                    "Cues bind to displays by position on the canvas, not a DisplayId field. Output is any extra OS screen (Win+P Extend). Pick the wall/TV, not the Producer laptop. Esc closes Output. A D3D11 compositor decodes each H.264 file once (Media Foundation / DXVA) and draws the wall from that texture. While Output is live, Stage stays a labeled canvas (Display 1 in the center) so the laptop does not run a second 4K decode — that is why Stage looks black. Close Output to preview the clip on Stage again. If you load another video and the wall stays black, Stop then Play — WatchMe keeps the last frame while the new file opens, rebuilds a dead DXVA decoder, and writes the error in Log if the file will not decode. Output now fills a top-level wall HWND on the HDMI/DP screen (not a nested 64×64 window under WPF). Log should show “Output wall HWND …” and then “Output has picture on the wall” when Play has a frame. Prefer H.264 MP4.\n\n" +
