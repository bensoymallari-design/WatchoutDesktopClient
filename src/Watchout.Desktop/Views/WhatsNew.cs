        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Timeline grab — the playhead and cue start/end edges use a left-right resize cursor and a ~10px hit so they are easy to drag (not a pointing arrow on a 1px line). The clip body uses a move cursor. Drag the ruler or the playhead triangle to scrub.\n" +
                    "MP4 fine for ~2 hours then stuck — Intel UHD EVR leaks on a long DXVA run; USB capture is a different HWND so it keeps running. WatchMe rebuilds only the file decoder after 50 min (Log: “Output MP4 DXVA rebuilt after 50 min… Capture is unchanged”) and still rebuilds if the last frame freezes.\n" +
                    "Output stuck with a picture — GPU-off DXVA (MediaElement) kept seeking a frozen EVR, which reset the stall clock, so the last frame never rebuilt. WatchMe now counts those catch-up seeks, Pause+Play kicks the decoder, and logs “Output stuck — … last picture frozen”. Prefer 8-bit H.264 if the clip is HEVC / HDR / Dolby Vision.\n" +
                    "Capture card running, MP4 stuck — USB/NDI is a layered HWND on Output. EVR exclusive overlay then freezes the file while the card keeps blitting. Output now opens the MP4 with ScrubbingEnabled (non-overlay renderer) whenever a live HWND is on that wall.\n" +
                    "Cue transitions — right-click the start of a timeline cue for fade / dissolve / wipe / scale in; right-click the end for the matching out. Properties has Start transition and End transition filter lists plus duration. Old Fade in / Fade out checkboxes still play as a linear fade.\n" +
                    "Stage column switches — Devices / Layers / Log / Switches. H9-style panel: HDMI/NDI cards with an ON/OFF toggle, STAGE screen tiles, orange LED lamps. Click a lamp to put a source on that Stage; click it again (or OFF) to take it off Stage.\n" +
                    ...
                    "Devices tab — Producer opens on Devices (screens, capture, audio) in the lower-right, not Layers. Layers, Log, and Switches are next to it. Switches is the H9-style HDMI/NDI to SCREEN matrix.\n" +
