        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "HAP encode on this PC failed with Unrecognized option chunks — ffmpeg aborted before writing HAP Q, so every MP4 stayed DXVA. WatchMe no longer passes -chunks, retries plain hap, and logs the real ffmpeg line (not the librubberband banner). Stage no longer covers Play with a gold name box when Output is live and the D3D11 compositor is off — MediaElement can show picture. D3D11CreateDevice uses the native feature-level count so Intel UHD is less likely to return E_INVALIDARG.\n" +
                    "Resolume Alley workflow — import encodes HAP Q off the UI thread (the “thread owns it” crash was aborting encode before ffmpeg ran, so Play stayed on heavy DXVA). Play is a GPU DXT blit like Arena; Stage is a 1280-wide preview, not a 4K RAM Map. RAM-full on an 8 GB laptop stays on the meter and does not flood the Log. A Windows screenshot no longer closes WatchMe when Intel GPU memory is gone.\n" +
                    "Play with no image and no black/stuck Log — the D3D11 compositor failed at boot (Intel UHD E_INVALIDARG from an empty feature-level list). Present never ran, so the diagnostic never ran, and an empty top-level wall HWND covered the WPF picture. WatchMe now creates the device with feature levels 11.1–10.0, retries, logs “Output black — D3D11 compositor…”, drops that covering HWND, and keeps Stage as a corner label instead of a gold canvas over the clip. Not RAM.\n" +
                    "Resolume Alley HAP Q — a YouTube title like “Dolby Vision” is not HDR. yt-dlp WebM → HandBrake MP4 plays in Resolume as HAPQ 4264 (GPU DXT, RAM 83%, FPS 30). WatchMe now encodes HAP Q on import (4K included) and Play uploads DXT to the GPU compositor — no RGB32 RAM copy. Log: “HAP GPU texture — … Resolume Alley / HAP Q”. Original MP4 still plays until HAP is ready. ffmpeg on PATH is required for the encode.\n" +
                    "RAM Tight is not a black Output — 6.8–7.0 / 7.7 GB sitting on the 88% line used to spam Tight / recovered in the Log so it looked like the laptop could not play 4K. RAM-only Tight stays on the meter (close browsers; this 4K is still OK). The Log keeps GPU/CPU/full warnings, and Output black / stuck lines. Resolume on the same PC is the proof.\n" +
                    ...
                    "Resolume GPU path — H.264 DXVA frames stay on the GPU (DXGI surfaces) and composite in D3D11. WatchMe no longer copies every file frame through RGB32 system RAM then uploads it again. That copy is why RAM filled and the Colorlight X20 hitch after a long PLAY while the RAM bar was still green. NDI and capture still upload from CPU (they arrive as pixels). Log: “DXVA GPU texture — … stays on the GPU (Resolume path)”. Intel UHD can still fall back to RGB32 if hardware surfaces fail.\n" +
                    ...
                    "CPU / GPU / memory — left-to-right CPU · RAM · GPU bars. Green = room for more. Amber RAM means close browsers — it does not mean this laptop cannot run 4K (Resolume on the same PC is the proof). Amber GPU/CPU means another 4K may hitch. Red = full. H.264 now stays on the GPU like Resolume; NDI/capture still copy pixels in RAM.\n" +
