        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Stage picture — a live Output used to leave Producer Stage as a black 0.15s poster while the GPU compositor was off (Intel UHD). Pause at 7 s still showed that first-frame JPEG. Stage now plays the same H.264 with MediaElement so the cue interior is the video, not a black box. One decode when D3D11 is up; dual decode only as the GPU-off fallback. Gold title bar still sits above the HWND.\n" +
                    ...
                    "Client Play — WatchMe stays up if DXGI/D3D11/Media Foundation throws (screenshot, GPU full, E_FAIL). While PLAY is running, a failed compositor is not retried every 2 s (that covered the wall and could crash). Output keeps one Media Foundation decode; Stage plays the clip when the compositor is off so Producer is not a black box. HEVC/HDR files warn at import instead of going black on the client wall.\n" +
                    "Stable Play — import links the HandBrake 8-bit H.264 MP4 and plays it. No background HAP encode (that fought Play on 8 GB Intel UHD and aborted ffmpeg with Unrecognized option chunks). One DXVA decode when the D3D11 compositor is up. If it is off, Stage still shows the video (MediaElement) — not a black poster. Log: “Play — one H.264 decode (DXVA)”. Assets → Encode HAP stays as an optional menu.\n" +
                    ...
                    "Stage / cue resize — Fit-to-display clips no longer snap back to the wall (the drop magnet was 200+ px at overview zoom). Drag and resize use a ~12 px snap. While Output is live and the GPU compositor is up, Stage shares that decode; if the compositor is off, Stage still plays the clip so Producer is not a black box.\n" +
