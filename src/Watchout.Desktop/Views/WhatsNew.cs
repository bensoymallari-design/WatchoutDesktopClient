        ...
        Content = new ScrollViewer
        {
            ...
            Content = new TextBlock
            {
                ...
                Text =
                    ...
                    "Stage cue names — each clip on Producer Stage has an opaque white-on-black title bar and a gold outline so you can read the name when the picture is black. The wall itself stays unlabeled.\n" +
                    "Client Play — WatchMe stays up if DXGI/D3D11/Media Foundation throws (screenshot, GPU full, E_FAIL). While PLAY is running, a failed compositor is not retried every 2 s (that covered the wall and could crash). Output keeps one Media Foundation decode; Stage stays a poster. HEVC/HDR files warn at import instead of going black on the client wall.\n" +
                    "Stable Play — import links the HandBrake 8-bit H.264 MP4 and plays it. No background HAP encode (that fought Play on 8 GB Intel UHD and aborted ffmpeg with Unrecognized option chunks). One DXVA decode on Output. If the D3D11 compositor is off, Stage shows a poster still — not a gold name box and not a second 4K MediaElement. Log: “Play — one H.264 decode (DXVA)”. Assets → Encode HAP stays as an optional menu.\n" +
                    ...
                    "Play with no image and no black/stuck Log — the D3D11 compositor failed at boot (Intel UHD E_INVALIDARG from an empty feature-level list). Present never ran, so the diagnostic never ran, and an empty top-level wall HWND covered the WPF picture. WatchMe now creates the device with feature levels 11.1–10.0, retries, logs “Output black — D3D11 compositor…”, drops that covering HWND, and keeps Stage as a poster / corner label instead of a gold canvas over the clip. Not RAM.\n" +
                    ...
                    "Stage / cue resize — Fit-to-display clips no longer snap back to the wall (the drop magnet was 200+ px at overview zoom). Drag and resize use a ~12 px snap. While Output is live, Stage does not run a second 4K decoder (GPU preview or a poster still).\n" +
