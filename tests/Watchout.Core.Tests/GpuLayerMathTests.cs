...
public class GpuLayerMathTests
{
    ...
    public void OutputPresentsAtTheScreenSizeNotThe64HostStub()
    {
        ...
        Assert.Equal(1, fittedOnSmallerWall.ScaleX, 3);
        ...
        // Resolume: Composition 516×430 on Screen 1920×1080 → 1:1 top-left, black around.
        var resolume = OutputViewMath.OutputViewport(0, 0, 516, 430, 1920, 1080);
        Assert.Equal(1, resolume.ScaleX, 3);
        Assert.Equal(1, resolume.ScaleY, 3);
        var slice = GpuLayerMath.MapRect(new StageRect(0, 0, 516, 430), resolume.OriginX, resolume.OriginY, resolume.ScaleX, resolume.ScaleY);
        Assert.Equal(0, (int)Math.Round(slice.X));
        Assert.Equal(0, (int)Math.Round(slice.Y));
        Assert.Equal(516, (int)Math.Round(slice.W));
        Assert.Equal(430, (int)Math.Round(slice.H));
